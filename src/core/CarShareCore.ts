...
import { validators } from './validators/carShareValidators';

dayjs.extend(utc);

class CarShareCore extends AppCore {
  constructor(props: BaseCorePropsInterface) {
    super({
      ...props,
      gatewayName: 'carShareGw',
      name: 'CarShare',
      hasOrderNo: false,
      doAuth: true,
      doingWhat: {
        list: 'listing car shares',
        get: 'getting a car share',
        getMany: 'getting multiple car shares',
        create: 'creating a car share',
        createMany: '',
        update: 'updating a car share',
        updateMany: '',
        set: '',
        remove: 'removing a car share',
        removeMany: 'removing multiple car shares',
      },
    });
  }
  ...
  /**
   * Verify car share belongs to current account via car
   */
  private async verifyRecordOwnership(recordId: string): Promise<{ isOwned: boolean; record: any }> {
    const { accountId } = this.getContext();
    const record = await this.getGateways().carShareGw.get(recordId);
  ...
  public async afterGetMany(items: any, opt?: BaseCoreActionsInterface): Promise<any> {
    ...
    // Batch fetch cars for all shares
  ...
  public async afterCreate(items: any, opt?: BaseCoreActionsInterface): Promise<any> {
    // TODO: Send notification to toUserId about the share invitation
  ...
  public async beforeUpdate(params: any, opt?: BaseCoreActionsInterface): Promise<any> {
    ...
    if (!id) {
      return OpResult.fail(OP_RESULT_CODES.NOT_FOUND, {}, 'Car share ID is required');
    }

    // Verify record ownership via car
    const { isOwned, record } = await this.verifyRecordOwnership(id);
    if (!isOwned) {
      return OpResult.fail(OP_RESULT_CODES.NOT_FOUND, {}, 'Car share not found');
    }

    // Don't allow changing carId, fromUserId, toUserId
    const { carId: _, fromUserId: __, toUserId: ___, ...restParams } = params;
